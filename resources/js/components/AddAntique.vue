    ...
    <Teleport to="body">
        <div v-if="show" class="modal-overlay" @click.self="closeModal">
            <div class="modal-content">
                ...
                <h2 class="form-title">Ajouter une Antiquité</h2>

                <div v-if="error" class="alert alert-danger">
                    {{ error }}
                </div>

                <form @submit.prevent="addAntique" enctype="multipart/form-data">
                    ...
                    <div class="form-buttons">
                        <button 
                            ...
                            :disabled="loading"
                        >
                            {{ loading ? "Envoi en cours..." : "Ajouter l'antiquité" }}
                        </button>
...
async function addAntique() {
    try {
        ...
        if (!token && !window.Laravel?.isLoggedIn) {
            alert("Veuillez vous connecter pour ajouter une antiquité.");
            closeModal();
            router.push("/login");
            return;
        }
        ...
        if (antique.value.image) {
            formData.append("image", antique.value.image);
        }

        await api.get("/sanctum/csrf-cookie");

        const response = await api.post("/api/antiques", formData, {
            headers: {
        ...
        if (response.data.success || response.status === 201) {
            alert("Antiquité ajoutée avec succès!");
            emit('success');
            closeModal();
        } else {
        ...
        if (err.response?.status === 401) {
            ...
            error.value = err.response?.data?.message || "Erreur lors de l'ajout de l'antiquité";
        }
    } finally {
        loading.value = false;
    }
